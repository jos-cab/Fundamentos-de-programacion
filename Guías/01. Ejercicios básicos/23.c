...
Vamos a suponer que nuestro ascensor puede soportar un máximo de 400 kg y hasta
6 personas.

Nuestra simulación debe proceder del siguiente modo:
- A medida que las personas ingresan al ascensor de a una a la vez, se registra
el peso de la persona.
- Supondremos que el ingreso de 0 kg, indica que no hay más personas por subir
al ascensor.
...
Por último, si habiéndose indicado que todas las personas están a bordo del
ascensor  y las condiciones establecidas se cumplen, el ascensor anunciará
...
int main()
{
    ...
    do
    {
        ...
        peso_total += peso;
             ...
             (cantidad_personas <= CANTIDAD_PERSONAS_MAXIMO) &&
             (peso_total <= PESO_MAXIMO));
